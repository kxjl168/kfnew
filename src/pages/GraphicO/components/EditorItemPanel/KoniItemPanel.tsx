import { Item, ItemPanel } from 'gg-editor';

import { Card, Input, Button, Col, Row, Spin } from 'antd';
import React, { useState, useEffect, useRef } from 'react';
import './index.less';

import GraphicItem, { GraphicItemProps } from './GItem';
import concat from 'lodash/concat';
import { Link, connect, Dispatch } from 'umi';

import { ConnectState } from '@/models/connect';
import logo from '@/assets/graphic.svg';

import { GraphicState } from '@/models/graphic';
import { SearchOutlined, DoubleRightOutlined, DoubleLeftOutlined } from '@ant-design/icons';


interface KoniItemProps {
  dispatch: Dispatch;
  graphicModel: GraphicState;
  initFun?: boolean;
}


const KoniItemPanel: React.FC<KoniItemProps> = (props) => {

  // const [gitems, setGitems] = useState<GraphicItemProps[]>([
  //   <GraphicItem key="1" shape='custom-node-circle' kclass='custom-node1' label='测试节点1' />,
  //   <GraphicItem key="2" shape='custom-node-rect' kclass='custom-node2' label='测试节点2' />,

  //   <GraphicItem key="3" shape='CustomNodeLRect' kclass='custom-node3' label='测试节点3' />,

    const[loading,setloading]=useState<boolean>(false);
  // ]);

  const { graphicModel, initFun } = props;

  //console.log(`11:${JSON.stringify(graphicModel)}`);

  const [init, UpdatInit] = useState<boolean>(false);
  const myRef = useRef();

  const loadnode = () => {

    let all = gitems;
    for (let index = 0; index < 2; index++) {
      let g = <GraphicItem shape='custom-node-rect' kclass='custom-node1' label={`新增节点${index}`} />;
      all = concat(all, g);

    }
    // console.log(all);
    setGitems(all, () => {
      // console.log(' loadnode node done...')
      // console.log(gitems);
    });

  };


  useEffect(() => {


    props.dispatch({
      type: 'graphic/setqueryname',
      payload: {
        name: ""
      }
    });


    if (!init) {
     // console.log('start init node...')

      props.dispatch({
        type: 'graphic/reload',
        payload: {
          num: 4,
          a: '1',
        }
      })


      UpdatInit(true);
    }
  }, []);




  //useEffect(() => {
  //  if (graphicModel.nodeitems) {
  //   console.log('reload node...')
  // let all = [];
  // for (let index = 0; index < graphicModel.nodeitems.length; index++) {
  //    const { shape:cshape,kclass: ckclass, label:clabel } = graphicModel.nodeitems[index];
  //    let g = <GraphicItem key={`key${index}`} shape={cshape} kclass={ckclass} label={clabel} />;
  //    all = concat(all, g);
  //  }

  // setGitems(all);

  // }
  // }, graphicModel.nodeitems);




  return (
    <>
    
      
    <ItemPanel className="itemPanel">
      <Card bordered={false}>
        {/* <Item
        type="node"
        size="40"
        shape="koni-custom-node"
        model={{
          color: '#69C0FF',
          label: 'Bank',
          labelOffsetY: 28,
          icon:
            'data:image/svg+xml;base64,',
        }}
        src="data:image/svg+xml;base64,"
      />
      <Item
        type="node"
        size="40"
        shape="koni-custom-node"
        model={{
          color: '#5CDBD3',
          label: 'Person',
          labelOffsetY: 28,
          icon:
            'data:image/svg+xml;base64,',
        }}
        src="data:image/svg+xml;base64,"
      />
      <Item
        type="node"
        size="40"
        shape="koni-custom-node"
        model={{
          color: '#B37FEB',
          label: 'Country',
          labelOffsetY: 28,
          icon:
            'data:image/svg+xml;base64,',
        }}
        src="data:image/svg+xml;base64,"
      /> */}

        <>
          {/* {
            gitems
          } */}
          <div className="flex lquery">
          
              <Input placeholder={ graphicModel.graphicType==="cls"?"输入概念名称":'输入实体名称' } ref={myRef} onKeyUp={(e) => {
                //  console.log(myRef.current.input.value);
                props.dispatch({
                  type: 'graphic/setqueryname',
                  payload: {
                    name: myRef.current.input.value
                  }
                });


                  if(e.key==='Enter')
                  {
                    setloading(true);
                    props.dispatch({
                      type: "graphic/reload",
                      payload: {
                        //  clsName: this.props.graphicModel .queryname
                      }
                    })
                  }

                  setTimeout(() => {
                    setloading(false);
                  }, 1000);
                

              }} />
              {loading&&<Spin size="small"  />}
              
           
          
              <Button title="查询" className="minbutton"  onClick={() => {
                props.dispatch({
                  type: "graphic/reload",
                  payload: {
                    //  clsName: this.props.graphicModel .queryname
                  }
                })

              }

              }>
                <SearchOutlined />
              </Button>
           
          </div>

          {
           // debugger;
            graphicModel.nodeitems.map((item) =>
              <GraphicItem key={item.uid} keyv={`key${item.uid}`} nodetype={item.nodetype} model={item.model} shape={item.shape} kclass={item.kclass} label={item.label} />
            )
          }

        </>


      </Card>
    </ItemPanel>
    </>
  );
};

//export default connect(()=>{})(KoniItemPanel);

export default connect(({ graphic, loading }: ConnectState) => ({
  graphicModel: graphic,
  initFun: loading.effects['graphic/init'],
}))(KoniItemPanel);
