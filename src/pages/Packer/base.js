...
base2.code.Colorizer.addScheme("xml", {
        attribute: /(\w+)=("[^"]*"|'[^']*')/,
        cdata: /<!\[CDATA\[([^\]]|\][^\]]|\]\][^>])*\]\]>/,
        comment: /<!\s*(--([^-]|[\r\n]|-[^-])*--\s*)>/,
        entity: /&#?\w+;/,
...
//with (base2)
base2.code.Colorizer.addScheme("html", {
        conditional: /<!(--)?\[[^\]]*\]>|<!\[endif\](--)?>/,
...
//with (base2.code.Colorizer)
base2.code.Colorizer.html.merge(xml);
//with (base2.code.Colorizer)
base2.code.Colorizer.  addScheme("css", {
        at_rule: /@[\w\s]+/,
        bracketed: /\([^'\x22)]*\)/,
...
//with (base2.code.Colorizer)
base2.code.Colorizer.addScheme("javascript", {
...
//with (base2)
   // with (code)
    base2.code.Colorizer["html-multi"] = Colorizer.html.union({
            inline: function(a, b, c, d) {
...
//with (base2.code.Colorizer.javascript) {
    base2.code.Colorizer.javascript.add("\\b(" + (base2.exports + ",base,base2,merge,union,implement,Array2,Date2,String2").match(/[^\s,]+/g).join("|") + ")\\b", '<span class="base2">$0</span>');
