...
import { query, get } from '../../pages/KgClassList/service';
import { FormInstance } from 'antd/lib/form';
import { isnull } from '@/utils/utils';
import { PlusCircleOutlined } from '@ant-design/icons';
import CreateForm from '@/pages/KgClassList/components/CreateForm';
import ProTable from '@ant-design/pro-table';
import { handleAdd } from '@/pages/KgClassList';
...
const Comp: React.FC<SelectProps> = (props) => {
    ...
    const { placeholder, subKgId, style,onlyreal, onChange, selectVal, labelInValue, config, className, hiddenAddBtn, mutiSelect, hideAddMore } = props;
        ...
        fetch(txt, data => {
            ...
            if (data && data.data)
                data.data.forEach((item, index) => {
                    val.push({ text: item.clsName, value: item.id });
                })
            setData(val);
            setTimeout(() => {
                // setValue(selectVal);
    ...
    useEffect( () => {
        ...
        if (selectVal && (selectVal.length > 0) && (!isnull(selectVal[0].text))
            ...
            if (selectVal instanceof Object) {
                setValue(selectVal);
                return;
            }
            try {
                const s= JSON.parse(selectVal);
                if(s)
                {
                setValue(s);
                return;
            ...
            // setTimeout(async() => {
            //     const clsrst = await get(selectVal);
            //     if (clsrst && clsrst.success) {
            //         if (labelInValue)//获取name
            //            {
            //             const newval={ label: clsrst.data.clsName, value: clsrst.data.id, key: clsrst.data.id, text: clsrst.data.clsName };   
            //             setValue(newval);
            //            if (onChange)
            //            onChange(newval);
            //     }
            //         else {
            //             setData({ label: clsrst.data.clsName, value: clsrst.data.id, key: clsrst.data.id, text: clsrst.data.clsName });
            //             setValue(selectVal);
            //             if (onChange)
            //             onChange(selectVal);
            //         }
    ...
    const fetch = (iputval, callback) => {
        SetFetching(true);
        let querydata = { dirId: querySubKgIds,curUser: onlyreal?"-1":'' };
        if (iputval)
            querydata.clsName = iputval.trim();
    ...
    const refreshValue = (fdata, uid) => {
        //更新数据源options
        let newdata = data.concat({ text: fdata.clsName, value: uid });
        setData(newdata);
        ...
        if (!mutiSelect) {
            //单选替换
            if (labelInValue)
                newvalue = { label: fdata.clsName, key: uid, text: fdata.clsName, value: uid };
            ...
            if (labelInValue) {
                newvalue = value.concat({ label: fdata.clsName, key: uid, text: fdata.clsName, value: uid });
    ...
    return (
        <div className="attrSelect">
            ...
            <Button title="新增概念" hidden={hiddenAddBtn} onClick={() => {
                SetAttrModelVisible(true);
            }}>
                <PlusCircleOutlined />
            </Button>
            ...
            <CreateForm
                ...
                title="新建概念" onCancel={() => SetAttrModelVisible(false)} modalVisible={attrModelVisible} />
